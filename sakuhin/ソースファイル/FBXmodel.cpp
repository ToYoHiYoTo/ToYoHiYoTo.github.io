	...
	{		
		...
		for (int m = 0; m < m_Scene->mNumMaterials; m++)
		{
			...
			if (m_Scene->HasTextures())
			{
				...
				//外部パスだったら名前が入る||埋め込みだったら先頭に'*'が入る
				Material->GetTexture(aiTextureType_DIFFUSE, 0, &texture_file);

				//埋め込みテクスチャ
				if (texture_file.data[0] == '*')
				{
					m_Texture->LoadFromMemory(m_Scene->mTextures[0]->pcData, m_Scene->mTextures[0]->mWidth);
				}
				else
				{
...
void FBXmodel::DrawShadow(XMMATRIX & Matrix)
...
void FBXmodel::DrawMeshShadow(aiNode * Node, XMMATRIX & Matrix)
